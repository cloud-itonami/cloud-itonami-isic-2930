(ns autoparts.operation
  "OperationActor -- one auto-parts-manufacturing operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Auto-Parts Advisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Auto-Parts
  ...
  One graph run = one auto-parts-manufacturing operation (intake ->
  ...
  (:require [langgraph.graph :as g]
            ...
            [autoparts.autopartsadvisor :as autopartsadvisor]
...
(defn build
  ...
  `autoparts.store/Store`).
  opts:
    :advisor      -- an `autoparts.autopartsadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (autopartsadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Auto-Parts Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (autopartsadvisor/-advise advisor store request)]
            {:proposal p :audit [(autopartsadvisor/trace request p)]})))

      ;; Auto-Parts Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
